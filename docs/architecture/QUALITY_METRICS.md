...
**What it does**: This is a smoke check of the quality aggregator, not a measurement suite. The script feeds a fixed mock `PipelineResult` (3 successful stages, `renderTime: 0`) into `qualityMonitor.assessPipelineQuality()` and prints the resulting overall / performance / accuracy / reliability scores. It verifies the aggregation path runs end to end; it does **not** execute real transcription, WER calculation, entity/relationship accuracy comparison, layout overlap detection, rendering, or performance benchmarks.

**Where real measurements come from**: End-to-end runs with real audio (e.g., the Phase 29 run on `public/jfk.wav`). Per-phase results — including recent success rates (Phase 43: 85.7%, Phase 44: 50.0%–66.7%) — are recorded in [ITERATION_LOG.md](./ITERATION_LOG.md).

**Frequency**: On demand (`npm run quality:check`). It is not wired into CI. The CI pipeline (`.github/workflows/ci.yml`) runs lint / type-check / tests / build / multi-seed fuzz, but not this script.
...
- **File**: `docs/architecture/ITERATION_LOG.md` (default path of `IterationLogger` in `src/utils/iteration-logger.ts`)
- **Format**: Markdown with structured metadata, grouped by phase section
- **Retention**: Last 100 iteration entries (older entries are trimmed automatically)

**Example Entry** (as generated by `IterationLogger`):
```markdown
