...
- [x] **TC-376-01**: REQ-392 producer census completeness/stale/reason — `tests/guards/optional-metric-producer-census.test.ts` が生産 surface 全体（repo src/ + @stv-core core-four）の optional numeric 宣言（`field?: number`）から measurement token（REQ-391 語彙 + f1/completeness）を持ち config token を持たない field を発見し (a) 発見された全 declaration が ROSTER 32 row に分類済みであること（未分類 = 未populate契約の新規 ship で RED）(b) LIVE / INPUT-CONTRACT row の宣言消失は stale row として RED (c) 全 row が verdict + 非空 reason を持つこと (d) INPUT-CONTRACT row は producer-free を維持し producer 出現で LIVE 昇格（`PipelineProgress.initialQualityScore` — SSR/test 用 presentational prop）。🔵 **Phase 190（TASK-0274）履行**: census 8/8 GREEN・MW-056 mutation (a) で eradicated-reappear も completeness と二重捕捉 — revert GREEN🔵 *Phase 190 (TASK-0274 / MW-056) で完了*
