...
- [ ] **TC-410-01**: REQ-426 ビデオプレビュー結線 — VideoPreview が reachable tree に mount され、`SpeechToVisualsVideo` composition の in-app preview（再生制御・seekbar 含む）が page test で立証されること。検証コマンド（履行時）: page test が RED→GREEN。🔵 *未実施（REQ-426 要件化のみ・次サイクル実装）*
- [ ] **TC-410-02**: REQ-426 単一ソースと死 import 解消 — preview が既存 `SpeechToVisualsVideo` composition を再利用していること（第二 composition 実装の不存在）・`VideoRenderer.tsx:10` の未使用 `Player` import が解消されていること。検証コマンド（履行時）: source pin / import 検査 test が RED→GREEN。🔵 *未実施*
