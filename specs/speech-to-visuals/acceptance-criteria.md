...
- [ ] **TC-423-01**: REQ-430 (a)(b) 減点の単一ソース適用 — recovery chain が disclosed-placeholder step で終端した run（`getRecoveryOutcome()` の `winningStepId` が disclosed-placeholder・transcriber.ts:183 `isFallback` と同一権威）において、正典 estimator（quality-estimators.ts）が減点後の transcriptionAccuracy を返し、MainPipeline / SimplePipeline / FrameworkIntegratedPipeline の 3 経路すべてがその値を品質集計に採用すること（segment 固定文の文字列照合などの第二推定経路を作らない）。検証コマンド（履行時）: 全 ASR step 失敗を注入した pipeline test が減点値を assert して RED→GREEN。🔵 *未実施*
- [ ] **TC-423-02**: REQ-430 (c) fail-closed 閾値 — 減点後の transcriptionAccuracy が改善閾値 0.85 未満であることを pin する test を持ち、減点値が named constant として定義されていること（裸 literal は grep / census 検査で拒否）。検証コマンド（履行時）: 閾値未満 pin test + constant 定義 grep。🔵 *未実施*
- [ ] **TC-423-03**: REQ-430 (d)(e) 非 placeholder 不影響・契約不変 — 実 ASR step（whisper 推論・Web Speech file）が成功した run の transcriptionAccuracy は現行値のままであること・`TranscriptionResult` 形状と `transcribe()` signature および既存 barrel 経由 test pin が本変更で崩れないこと。検証コマンド（履行時）: 既存 transcription test suite 全 green + 成功経路値 pin test。🔵 *未実施*
- [ ] **TC-423-04**: REQ-430 (f) mutation witness — 減点適用の除去（estimator から減点分岐の削除・または 3 経路のいずれかの消費忘れ）で TC-423-01/02 が isolated RED になることの実測（revert GREEN）。検証コマンド（履行時）: mutation → RED → revert → GREEN。🔵 *未実施*
