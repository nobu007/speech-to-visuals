...
- [x] **TC-355-03**: 触れた全 suite と型チェックが GREEN であること — 触 14 ファイル 16 suites / 3216 passed / 17 skipped（skip は spine-manifest の SPINE_EXISTS 条件付き・従来どおり）+ guards 76 suites / 3237 tests GREEN（census 14/14 含む）・`node_modules/.bin/tsc -p tsconfig.test.json --noEmit` で新規 error 0（既知 baseline 14 は不変であること）・`npm run specs:mirror:check` が exit 0（REQ-362 挿入位置は mirror 対象節の外）であること（REQ-362）🔵
