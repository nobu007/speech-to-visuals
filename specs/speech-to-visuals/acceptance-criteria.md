...
- [ ] **TC-424-01**: REQ-431 (a)(b) producer の単一ソース報告 — 3 経路（MainPipeline / SimplePipeline / FrameworkIntegratedPipeline）の `recordPipelineQuality` site が、品質集計に採用したのと同一の `estimateTranscriptionAccuracy` 出力（`endedAtDisclosedPlaceholder` context 付き・減点後）を monitor へ報告すること。monitor call site での再計算・`??`/fallback default 注入・第二推定経路が存在しないこと。検証コマンド（履行時）: 減点 context を注入した pipeline test が `getSnapshot().quality.transcriptionAccuracy` の減点値を assert して RED→GREEN。🔵 *未実施*
- [ ] **TC-424-02**: REQ-431 (c)(f) finite-or-null 契約と契約不変 — 報告前は `null`・NaN ingestion は `null`（sanitizeFinite chokepoint）・last report wins・`reset()` で `null` に戻る・failure path など測定が走らなかった run は報告なし（`null` 維持）であること。`PerformanceSnapshot.quality.transcriptionAccuracy` は `number | null` 型のまま・既存 `recordPipelineQuality(measuredScenes, layoutOverlapCount)` 呼び出し（test pin 含む）と `PipelineQualityReport` 既存 2 field が崩れないこと。検証コマンド（履行時）: monitor 契約 test（REQ-372 の 7-leg と同構造）が RED→GREEN + 既存 monitor suite 全 green。🔵 *未実施*
- [ ] **TC-424-03**: REQ-431 (d)(e) gate 挙動と scope 外維持 — 実測報告後 adaptive Transcription Accuracy gate（gte 0.85・blocker）が実値で評価され placeholder 終端 run（0.5）が実値 fail として検出されること（METRIC UNAVAILABLE ではない）・`updateAdaptiveThresholds` が null round skip を維持したまま実 round のみ学習すること・`quality.avgSceneQuality` は報告後も `null` fail-closed のままであること。検証コマンド（履行時）: adaptive-gates × monitor 連結 test が RED→GREEN。🔵 *未実施*
- [ ] **TC-424-04**: REQ-431 (g) mutation witness — producer 報告の除去（または 3 経路いずれかの報告忘れ）と monitor 側への捏造定数再注入（`transcriptionAccuracy: 0.90` 等）が isolated RED になることの実測（revert GREEN）。検証コマンド（履行時）: mutation → RED → revert → GREEN。🔵 *未実施*
