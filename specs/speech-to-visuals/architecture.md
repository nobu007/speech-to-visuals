...
**作成日**: 2026-04-27
**最終更新**: 2026-08-27（第237回検証: stv-core コア分割（PR #7・2026-08-18）の設計正本同期 — ディレクトリ構造から消滅済み src/config・src/lib・src/types・src/utils を除去し19ディレクトリに正規化・外部コアパッケージ @stv/core 境界 section 新設・出典パス62箇所を @stv/core 系へ正規化・361ファイルが import（2026-08-27実測）・REQ-420・TASK-0312）
**最終更新**: 2026-08-27（kairo-design session A127 統合記録 — INV-CACHE-002 を cache integrity contract 拡張として昇格・MW-085〜091 mutation matrix 構造化・CorruptionOverlay 4 hop 連結・PINNED_MIN_ENTRIES bump discipline 警告）/ 2026-08-23（A126 audit-pass-first census 独立節追加・Phase 189-193 REQ-391〜397）/ 2026-08-06（第209回検証: NaN/Infinityガンド横展開完了・clampFinite Infinity対応・DiagramVideo時間単位バグ修正・property-based fuzz tests追加・11+モジュール坚牢化・570ファイル・543テストファイル・107パッケージ・REQ-270~273）
...
- **DEFAULT フォールバック寸法解決ペアの単一ソース（round 49）**: レンダー既定フォールバックでの寸法解決2行ペア（width 軸 `getNodeWidth(node, DEFAULT_NODE_WIDTH)` + height 軸 `getNodeHeight(node, DEFAULT_NODE_HEIGHT)`）が16サイト×11ファイルに3イディオムで inline 再実装されていた族を、`src/visualization/node-dimensions.ts` の `defaultNodeExtent(node)`（`{width, height}` 返却・正典は getNodeWidth/getNodeHeight と DEFAULT_* 定数の直後 = primitives の隣）へ集約。イディオム: (1) two-const 前構え ×12（dagre-pipeline ×2 = g.setNode サイジング + positioned スタンプ・comparison/general/flow/matrix/tree スタンプ ×5・cycle ×2 スタンプ・network クランプサイジング・mindmap ×2 ブランチ）、(2) inline property 読み ×2（strategy-selector フォールバックグリッド・timeline スタンプ）、(3) per-node maxima ×2（cycle リング半径 `Math.max(...nodes.map(...))` 両軸）、加えて strategy-graph `scaledNodeExtent` 内の raw pair（r42 正典のスケールド双生 — compose に切替）= 計16サイト+1 compose。**registry sweep が17サイト目を発見**: conceptmap level-width packing は**式引数** site（`node ?? { width: 0, w: 0 }` を渡す幅解決）で初回の identifier-only grep が見逃し — shape-level registry パターン `[^()]*` が補足（MISSED-SIBLING-SITE 教訓の今回の発見網は registry 自身・width 軸のみ委譲し nullish ガードと importance スケールは site 残置）。**PER-AXIS シームが核心（r47 教訓）**: DEFAULT は width 120 ≠ height 60 — 単一共有数への崩壊・軸 swap が全スタンプのサイジングを静かに壊すため解決はペアのまま（層2 witness: 寸法なしノード → 120×60 + `DEFAULT_NODE_WIDTH).not.toBe(DEFAULT_NODE_HEIGHT)`）。**零 delta**: 全委譲が retired 呼び出しと同一引数同一評価順序（Object.is）・maxima は反復順序保存・scaledNodeExtent compose は 200 サンプル corpus でビット一致。スコープアウト（文書化）: (1) 暗黙デフォルトイディオム `getNodeWidth(node)` — getNodeWidth 自身の default 引数が単一ソース（quality-estimators 等は合法・registry は明示 DEFAULT-literal 引数形式のみ ban）、(2) 0-fallback 測定読み `getNodeWidth(result[i], 0)`（timeline — r41 測定 vs レンダー分岐）、(3) flow/tree の `?.width ?? DEFAULT_NODE_WIDTH` optional-chained エッジ読み ×2 ずつ（r46 スコープアウト）、(4) layout-utils `fallbackWidth: number = DEFAULT_NODE_WIDTH` fallback-**引数** シグネチャ（r41 シーム — 変数で呼ぶため literal ban は発火しない）。検証: verbatim オラクル（two-const・inline・maxima・scaledExtent 旧本体を c5c21785 から凍結）× seeded fuzz（mulberry32 4921・DIM_VALUES 9種 {undefined,0,−3.5,NaN,±Infinity,60,120,1e15} × 240 + 優先順位ラダー 8）で Object.is 一致 + セマンティクス pin（per-axis witness・フォールバックチェーン優先順位 per axis・暗黙デフォルト等価・LIVE witness = CycleLayoutStrategy 単一ノード {x:900, y:510, width:120, height:60}・TimelineStrategy スタンプ {x:900, y:80=CANVAS_PADDING, width:120, height:60}）+ ソースアンカー（正典 raw pair exactly once・11ファイル委譲形状・retired 形状 ban・スコープアウト4種の現行 shape pin・dagre center→top-left 変換 local は r30 guard ため残置）。mutation 5種 RED 検証: M1 cycle へ two-const 再 inline = 2 fail・M2 正典軸 swap = 63 fail・M3 委譲 destructure の幅エイリアス脱落 = 1 fail・M4 scaledNodeExtent 再 inline = 3 fail・M5 timeline スタンプ軸 swap = 2 fail。**意識的テスト更新（full suite が捕捉）**: r47 guard の cycle sizing ピン（DEFAULT 明示読みの site 残置を主張する `getNodeWidth(nd, DEFAULT_NODE_WIDTH)` アンカー）は r49 委譲で形状が変わり追跡更新（r42 前例・旧肯定 pin は否定 pin に転換 = 移行完了主張・intent「center fold は委任・sizing 読みは可視」は不変）。registry r49 エントリ 1 rule 2 パターン（幅解決 `getNodeWidth(…, DEFAULT_NODE_WIDTH)`・高さ双子 — identifier 非依存 `[^()]*` で式引数 site も捕獲・node-dimensions のみ exclude・minSweptFiles 300）。GOTCHA: (a) identifier-only 引数 grep は式引数 site を見逃す — ban パターンは shape-level で書き発見は sweep に担わせる、(e) targeted `--testPathPatterns "visualization|strateg|dagre"` は tests/guards/*-single-source.test.ts のパスにマッチしない — 他 round の guard アンカー（委譲連鎖の追跡更新要否）は full suite か explicit 再実行で検証する、(b) 委譲コメントに retired 形状を引用すると層3 not.toMatch が自爆する — コメント文言は形状を含めない、(c) timeline witness の startY は max(40, …) でなく `CANVAS_PADDING`(80) 固定 — 期待値は実コード読みで確定、(d) node-dimensions は unstable_mockModule 部分 mock が存在しないため worker mock 追記不要（r41/r45/r47/r48 の layout-utils export と違い link-error 面ゼロ）。registry 46 エントリ・アグリゲータ 122 行 + family ファイル 41 個・計 2120 行。🔵 *src/visualization/node-dimensions.ts・dagre-pipeline.ts・strategy-selector.ts・strategy-graph.ts・strategies/{comparison,general,flow,matrix,tree,timeline,cycle,network,mindmap,conceptmap}-strategy.ts・tests/guards/default-node-extent-single-source.test.ts・tests/guards/frozen-literal-families/default-node-extent.ts より*
- **正方グリッド packing + セル中央スタンプの単一ソース（round 50）**: グリッドレイアウト骨格3式とセル中央スタンプ — 列導出 `max(1, ceil(√n))`（10サイト×8ファイル: ezo ×2 = network 初期化 + basic グリッド・NetworkLayoutStrategy 初期化・ConceptMapLayoutStrategy・LayoutOptimizer ×2 = optimizeMatrixLayout + improveMatrixGrid・FallbackLayoutStrategy matrix・advanced-layouts・GridSnapStrategy cell 上限・flow-strategy 行容量）、行導出 `max(1, ceil(n/cols))`（7サイト: ezo・ConceptMap・LayoutOptimizer improveMatrixGrid・Fallback spacingY 除数 inline・matrix・general・overlap-resolver）、aspect 列導出 `max(1, ceil(√(n·ratio)))`（3サイト: matrix・general・overlap-resolver — 全サイトが `n * ratio` の被演算子順）、セル中央スタンプ `origin + i·cell + (cell − extent)/2`（9サイト×両軸: ezo ×2・Network・ConceptMap・LayoutOptimizer ×2・Fallback・matrix 2-step cellX local・general スパイラル）— を `src/visualization/layout-utils.ts` の `squareGridColumns(count)` / `squareGridRows(count, columns)` / `aspectGridColumns(count, aspectRatio)`（columns 正典の compose・`count * ratio` 評価順保存）/ `centerInCell(index, cell, extent, origin = 0)` へ集約。**clamp は LIVE**（r48 ringAngle のデッドガード引退と対照）: `count: 0` が ezo basic グリッド・Fallback matrix に到達し、clamp なしは canvas/0 列の除算になる — `max(1,…)` は家族の verbatim 挙動として保持。**既存ドリフトを発見**: advanced-layouts の行導出は clamp 落下 `Math.ceil(nodes.length / cols)` で、かつ**一度も読まれないデッド変数**（ファイル内唯一の出現）だったため委譲ではなく引退（ringAngle デッドガンド引退の前例・live な再導入は registry sweep が捕捉）。**behavior change: スタンプは FP 再 grouping の ulp delta を伴う** — 引退サイトは代数的に等価な2 grouping を使い binary64 でビット非同一: A `(i·cell + cell/2) − extent/2`（ezo ×2・Network・LayoutOptimizer.optimizeMatrixLayout）vs B `i·cell + (cell − extent)/2`（ConceptMap・Fallback・improveMatrixGrid・matrix・general）。正典は B（strategy 層の多数派・「残り空間の半分」の意図）。A→B 移行4サイトは canvas ドメイン全域スイープ（1920×1080 級10種 × 列1–80 × extent 10種 × margin 3種 = 846,000組）で **max 9.1e-13 px**（約7.6%のサンプルが最下位 ulp クラスで変化・整数フレンドリーグリッドは完全一致）— レンダリング精度より8桁下で実際のノードサイズではオーバーラップ述語を反転させ得ない（layout-outcome-overlap-regression GREEN で確認）。fuzz 裾（i≤120・cell≤4000・extent≤900）では max 5.8e-11（≈7 result-ulp）を別 bound としてピン。**delta が実在すること自体を witness pin**（deltas > 0 — 空の bound は挙動変更を隠蔽する）。スコープアウト（文書化）: (1) overlap-resolver スナップスタンプ `col · cellWidth + 40`（中央項なしの origin-only — 折り込むと resolver のスナップ意味を変える）、(2) GridSnapStrategy のスパン中央 `(cellSize · cellsWide) / 2`（複数セル占有の別概念）、(3) advanced-layouts 固定ピッチ 200/150 スタンプ（extent 項なし）、(4) `floor(i/cols)`+`i%cols` divmod 誘導（整数演算で FP シームなし・flow は idx ソート順で被演算子が異なる）、(5) cell 寸法導出は site 残置（canvas/cols vs canvas/rows vs margin-inset vs 固定ピッチの意図的シーム — ezo:650 は rows 圧縮・ezo:576 は gridSize 両軸という同一ファイル内別ポリシー）。検証: verbatim オラクル（列・行・aspect・スタンプ A・B・matrix 2-step を 22980e5c から凍結）× seeded fuzz（mulberry32 5021/5022・canvas コーパス 846,000 + fuzz 裾 4,000）で Object.is（B 系・零 delta）or ≤1e-12px bound（A 系 canvas）/ ≤1e-9（fuzz 裾）+ セマンティクス pin（LIVE clamp witness 0→1 列/行・packing サイズ表・aspect 16:9 優位・スタンプ算術・LIVE witness = MatrixStrategy 単一ノード整数完全 {x:460, y:510}・GeneralStrategy スパイラル primitive 再合成）+ ソースアンカー（正典 raw 式 exactly once・11ファイル委託形状・advanced-layouts デッド行 ban・スコープアウト4種の現行 shape pin）。mutation 6種 RED 検証: M1–M4 禁止形状4種（列・行・スタンプ A・スタンプ B）の実コード行注入 = registry sweep が4 offender 全検出・M5 正典 `ceil→floor` + `centerInCell` B→A 退化 = 10 fail・M6 Network スタンプ委託 revert = registry + 層3アンカーの2系統 RED。registry r50 エントリ 1 rule 4 パターン（clamped-ceil-sqrt 前置き = 純正・aspect 両方を捕獲・`max(1, ceil(… / …))` 除数形状・スタンプ A は backreference `\1` で同一 cell を `+` 両側に要求・スタンプ B は `(cell* − …) / 2` — layout-utils のみ exclude・minSweptFiles 300）。GOTCHA: (a) `squareGridRows(5, 3)`=2（ceil(5/3)）— 期待値は手計算でなく正典定義から、(b) 層3 not.toMatch は委託コメントの引用で自爆（r49 教訓どおり codeLines() コメント除外で回避）、(c) アンカー regex の `[hg]` は `w` を捕獲しない — 文字クラスは実変数名を列挙する、(d) ワーカー2 mock への新 export 4件追記は r41/r45/r47/r48 と同様に両ファイル必須。registry 47 エントリ・アグリゲータ 124 行 + family ファイル 42 個・計 2158 行。🔵 *src/visualization/layout-utils.ts・enhanced-zero-overlap-layout.ts・strategies/{NetworkLayoutStrategy,ConceptMapLayoutStrategy,LayoutOptimizer,FallbackLayoutStrategy}.ts・advanced-layouts.ts・layout/strategies/GridSnapStrategy.ts・strategies/{flow,matrix,general}-strategy.ts・overlap-resolver.ts・src/workers/__tests__/{layout-delegation-helpers,layout-engine-integration}.test.ts・tests/guards/grid-packing-single-source.test.ts・tests/guards/frozen-literal-families/grid-packing.ts より*
...
- **型ガード・型安全性**: DiagramType（11種類）の実行時検証を行う isDiagramType() 関数により、不正な図解タイプ値を検出・排除 🔵 *@stv/core/types/diagram・要件定義REQ-051 より*
...
- **コード規模監査スクリプト**: ビルド時またはCI で SYSTEM_CONSTITUTION 制限値（ファイル数340以下・行数100K以下）を自動チェック 🔵 *@stv/core/config/code-size-audit・scripts/code-size-audit.ts より*
...
**信頼性**: 🔵 *@stv/core/config/limits・@stv/core/utils/sanitize・要件定義REQ-132~134 より*

Phase 52 で実装されたセキュリティ強化と設定集約:

- **集中制限設定**: ISS-044 対応。散在していたマジックナンバーを `@stv/core/config/limits` に集約。RATE_LIMITS（API: 100req/15min, UPLOAD: 20req/15min）、BATCH_LIMITS（MAX_CONCURRENT_JOBS: 3, MAX_STORED_JOBS: 200, MAX_FILES_PER_BATCH: 100）、PIPELINE_LIMITS（MAX_SCENES: 200, MAX_ITERATIONS: 500, MAX_OUTPUT_NAME_LENGTH: 255）、SECURITY_LIMITS（JWT_SECRET_MIN_LENGTH: 32）の4群定義。全定数 `as const` で型安全性確保 🔵 *@stv/core/config/limits より*
- **ファイル名サニタイズ**: `sanitizeFilename()` 関数によるパストラバーサル・nullバイト注入・制御文字攻撃対策。ディレクトリセパレータ(`/\`)→`_`置換、`..`除去、nullバイト除去、制御文字(0x00-0x1F, 0x7F)除去、先頭ドット除去、空結果フォールバック(`"unnamed"`) 🔵 *@stv/core/utils/sanitize より*
...
**信頼性**: 🔵 *@stv/core/utils/audio-duration・src/components/StageIndicator.tsx・@stv/core/config/limits・要件定義REQ-139~141 より*

Phase 54 で実装されたコンポーネント・ユーティリティテスト拡充:

- **音声時間計測ユーティリティ**: `getAudioDuration()` 関数（HTMLAudioElement loadedmetadata イベント・ObjectURL 生成/解放・preload='metadata' 最適化）と `formatDuration()` 関数（秒/分/時間フォーマット・Infinity/負値ガード）によるクライアントサイド音声時間取得 🔵 *@stv/core/utils/audio-duration より*
- **AUDIO_LIMITS 設定値**: 50MB 最大ファイルサイズと 3600秒（1時間）警告閾値の as const 定義。EDGE-103（1時間超音声の事前警告）の実装基盤 🔵 *@stv/core/config/limits AUDIO_LIMITS より*
...
**信頼性**: 🔵 *@stv/core/utils/audio-validation・src/components/SimplePipelineInterface.tsx・@stv/core/config/limits・要件定義REQ-142~143 より*

Phase 55 で実装されたパイプライン音声入力検証の統合:

- **validateAudioFile**: File オブジェクトのサイズ上限（EDGE-101: 50MB）・空ファイル検出（EDGE-001）・対応形式（MIME type + 拡張子）検証を一元化。SimplePipelineInterface のファイル選択ハンドラに統合 🔵 *@stv/core/utils/audio-validation より*
- **validateAudioDuration**: 音声再生時間の下限（EDGE-102: 1秒未満拒否）・長時間警告（EDGE-103: 1時間超過警告）・無効値（NaN/Infinity/負数）検出を一元化。SimplePipelineInterface の非同期チェックに統合 🔵 *@stv/core/utils/audio-validation より*
...
**信頼性**: 🔵 *src/components/ErrorAlertSystem.tsx・src/visualization/layout/OverlapResolver.ts・src/export/enhanced-export-engine.ts・@stv/core/utils/logger より*
...
**信頼性**: 🔵 *src/monitoring/health-check-service.ts・src/performance/real-time-performance-monitor.ts・src/monitoring/production-monitoring-excellence.ts・src/pipeline/actual-video-renderer.ts・src/pipeline/main-pipeline.ts・src/components/VideoRenderer.tsx より*
...
- **ProductionDashboard.tsx**: プロダクション設定ダッシュボード（設定管理・パフォーマンスレポート生成・リアルタイム監視・最適化ステータス・未保存変更追跡）🔵 *要件定義REQ-055・@stv/core/config/ より*
...
```
./
├── src/                    # 19ディレクトリ + エントリファイル（App.tsx/main.tsx 等・2026-08-27実測）🔵 旧 config/lib/types/utils は @stv/core 移管で消滅（PR #7・2026-08-18）
...
├── node_modules/@stv/core/ # 外部コアパッケージ（github:nobu007/stv-core#v1.0.7・git タグ完全 pin）— 共有型・ユーティリティ・設定・基盤 lib（下記「外部コアパッケージ」section）🔵
├── supabase/
...
**信頼性**: 🔵 *SYSTEM_CONSTITUTION.md V2.8 改正（2026-08-18 stv-core 分離）・package.json・note.md・第237回検証実測より*

- **移管対象**: 共有型（types/{diagram, pipeline, api, workspace, llm, cache, quality}）・ユーティリティ（utils/{logger, guards, sanitize, memory-usage, audio-validation, audio-duration, regex-escape, prometheus-label-escape, report-corruption, safe-storage, playback-time}）・設定（config/{limits, production-config, schema, validate, code-size-audit, env, errors}）・基盤 lib（lib/{metrics-utils, safe-array, capped-array, capped-map, unicode-script-ranges, quality-display-tiers}）を、コアパッケージ `@stv/core`（`github:nobu007/stv-core#v1.0.7`・git タグ完全 pin）へ移管（PR #7・2026-08-18）。プロダクトリポジトリの `src/types`・`src/config`・`src/lib`・`src/utils` は消滅し、`src/` 直下は19ディレクトリ + エントリファイルに縮小した。
- **依存の方向**: プロダクト → @stv/core の一方向。361ファイルが `@stv/core/*` から import（2026-08-27 実測・第237回検証）。@stv/core 側はプロダクト src に依存せず、独自ラチェット（6,000行/90ファイル上限）で自己拘束する（SYSTEM_CONSTITUTION V2.8 改正）。
- **単一ソース規則（REQ-310/312）**: 移管済みモジュールのプロダクト側再実装は禁止（単一ソース）。tests/guards が境界を structural pin で担保し、本設計書群の出典表記は `@stv/core/<area>/<module>` 形式（requirements.md 第218回検証と同一規約）に統一している。第237回検証で設計正本3文書（architecture.md・dataflow.md・interfaces.ts）の出典パス62箇所を正規化し、REQ-420 の design-doc-source-currency guard が再発を構造的に防止する。
- **バージョン運用**: 浮動 ref 禁止・GitHub タグ完全 pin（REQ-311）。バージョン更新は package.json と本 section の pin 表記を同一 commit で更新する（guard が不一致を検出）。
...
<!-- sync:mirror source-digest="56cab125a152" -->
...
**信頼性**: 🔵 *@stv/core/utils/guards・src/remotion/*.tsx・src/pipeline/*.ts・src/analysis/*.ts・直近6コミット(f42f7bc〜a68e5c2)より*

パイプライン全体でのNaN/Infinity伝播を防止する多層ガードシステム:

- **guards.ts中央集権化** 🔵: `sanitizeFinite()`（NaN/非数値→デフォルト値）・`clampFinite()`（±Infinityをそれぞれmax/minに変換）・`safeToLocaleString()`（undefined/NaN→'0'）の3つのユーティリティ関数に集約。今後の新規コードがガードなしで数値アクセスできないよう統一 *@stv/core/utils/guards*
...
**信頼性**: 🔵 *@stv/core/config/validate・@stv/core/config/schema・要件定義REQ-038 より*
