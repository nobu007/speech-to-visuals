...
**信頼性**: 🔵 *tests/guards/frozen-literal-rules.ts（round 34 時点 33 エントリ・1111 行。round 33 は 32 エントリ・1064 行。round 32 は 31 エントリ・1025 行。round 31 は 30 エントリ・956 行。round 30 は 29 エントリ・936 行、round 29 は 28 エントリ・892 行、round 28 は 27 エントリ・845 行、round 27 は 26 エントリ・810 行、方針確立時 round 16 は 16 エントリ・423 行。round 31 は新 family 1 件: v1 strategy shared members、round 32 は新 family 1 件: v2 strategy edge builder、round 33 は新 family 1 件: v1 engine edge builder（+ r32 エントリから v1 族6除外を解除）、round 34 は新 family 1 件: strategy edge repointing。次回再検討閾値は 1200 行（round 27 設定・残り89行、round 35 追加時に到達見込み））・frozen-literal-registry.test.ts より*

- **決定**: `tests/guards/frozen-literal-rules.ts` は**単一ファイル方針を維持**する。family 分割は行わない
- **800 行超過（round 27）の再検討記録**: round 16 が設けた 800 行の分割再検討閾値を round 27 の追記（810 行）で超過した。再検討の結果、**維持を再確認**: (1) 追加分は純粋にデータ（rule リテラル + 根拠コメント）であり、複雑度は行数ではなくエントリ数で増える — エントリ間に相互作用はない。(2) 分割案（family ごとのファイル + readFileSync 動的ロード）は走査エンジンの変更を伴い、frozen-literal-registry 全スイートの GREEN 再実行という検証コストを追加するが、読者にとっての利益は grep 可能な一覧が複数ファイルに散るだけ。(3) 本ファイルは append-only で推移し、衝突リスクは低い。**次回の再検討トリガーを 1200 行に設定**（エントリ数 ≒ 40 目安。到達時に再度 分割 vs 維持 を評価する）
