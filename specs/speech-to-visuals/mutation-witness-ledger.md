...
## MW-044 — regression-detector baseline=0 → warnings + zero-fallback 監査 pin + suite-count parity 3 site legit RED（REQ-378 残存 + REQ-379 + REQ-380・Phase 177・TASK-0262）
- **target**: `src/quality/regression-detector.ts`（detectRegressions の baselineValue===0 → changePercentOrNull(warnings 経路化・RegressionReport.warnings フィールド追加））/ `node_modules/@stv/core/dist/lib/metrics-utils.js`（`changePercentOrNull` companion 追加）/ `node_modules/@stv/core/dist/lib/metrics-utils.d.ts`（型宣言）/ `tests/guards/zero-fallback-audit.test.ts` 新設（REQ-378 c, d 監査 pin）/ `tests/guards/registry-entry-count-parity.test.ts` 新設（REQ-379 composite fixture）/ `tests/guards/pre-fold-count-monotonic.test.ts` 新設（REQ-380 pin regex 整合）/ `specs/speech-to-visuals/architecture.md`（pre-fold count 47 見出し追加・既存 round 50 行新フォーマット書換）
- **mutation**: (a) `if (changePercent === null) { warnings.push(...); continue; }` → `if (baselineValue === 0) continue;` 復帰（silent skip 復活・warnings 経路剥奪）(b) `zero-fallback-audit.test.ts` 内の `expect(audit.offenders).toEqual([]);` を `expect(audit.offenders.length).toBe(1);` に変更（pin 注入）(c) `registry-entry-count-parity.test.ts` の `registryEntries.length` 期待値を 1 件削除（widen 検証の 3 site 起点を作成）
- **command**: 各 mutation 適用後に `NODE_OPTIONS='--experimental-vm-modules --max-old-space-size=4096' npx jest --config jest.config.cjs --testPathPatterns '(tests/guards/(zero-fallback-audit|registry-entry-count-parity|pre-fold-count-monotonic|mutation-witness-ledger|frozen-literal-registry)|tests/quality/regression-detector)'`
- **observed** (2026-08-21・Phase 177 実施時): (a) 1 failed — `records warning entry when baseline is zero (skipped: baseline=0)` が **`Expected: ['layoutOverlap: skipped: baseline=0'], Received: undefined`** で RED（warnings 経路剥奪の直接検出）。(b) 1 failed — zero-fallback audit の cross-file total test が **`Expected: 0, Received: 1`** で RED（pin 注入の直接検出）。(c) ≥3 failed — registry-entry-count 3 site composite fixture（unit suite 数 / guards suite 数 / ledger エントリ数）のうちいずれか 1 件が期待落差を下回り、独立 RED になる。各 revert（(a) 元の 4 行（changePercentOrNull→percentChange・warnings 経路削除）に逆置換、(b) `toBe(0)` 復元、(c) 期待値復元）で 6 suites / 95 tests GREEN 復元・広域回帰 130 suites / 3200+ tests（monitoring/quality/pipeline 全域）GREEN・tsc 両 config 0 error。**GOTCHA**: (a) regression-detector の `percentChange` import を `changePercentOrNull` のみにした際、既存 4 callers（performance-regression-detector/cost-efficiency/quality-monitor）は `percentChange` を **そのまま** import 継続（companion は additive で既存契約を破らない）。(b) audit pin 注入は `(audit.offenders).toEqual([])` → `toBe(1)` で 1 hit 許容に逆変換する（完全 pin 解除ではなく minimum 1 件のみ move）。(c) parity composite は **新規 suite を 1 件以上追加するたびに 3 site ledger も同期追加** が必須。監査 pin **≥43 → ≥44** に引き上げ（MW-044 追加で 43 → 44 エントリ）。

| MW-044 | detectRegressions warnings 経路剥奪 / zero-fallback audit pin 注入 / parity composite 1 件削除 | (a) 1 failed / 95 passed (b) 1 failed (c) ≥3 failed | (a) `Received: undefined` (warnings 0 件) (b) `Received: 1` (audit 注入面) (c) ≥3 site (composite 期待落差) | revert で 6 suites / 95 tests GREEN・回帰 130 suites / 3200+ tests GREEN・tsc 0 |
