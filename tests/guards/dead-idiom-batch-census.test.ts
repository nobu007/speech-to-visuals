...
export const IDIOM_KINDS: readonly IdiomKind[] = [
  ...
  // --- REQ-419 tenth sweep (thirty-four kinds, all measured on 2026-08-26) ---
  // document.execCommand is the deprecated commands API (the clipboard
  // 'copy' dance included); navigator.clipboard is the form.
  { id: 'doc-execcommand', detect: /document\.execCommand\s*\(/ },
  ...
  // setImmediate is a NON-STANDARD scheduler (Node/IE legacy only) —
  // ReferenceError in the browser bundle; queueMicrotask is the portable
  // form. Same realm hazard as node-global-identifier.
  { id: 'nonstandard-setimmediate', detect: /(?<![.\w$])setImmediate\s*\(/ },
  // process.nextTick is Node-only in an isomorphic src tree — the same
  // browser-ReferenceError class as `global.` (queueMicrotask is the form).
  { id: 'node-process-nexttick', detect: /process\.nextTick/ },
...
const ALLOWED: Record<string, string> = {
  ...
  'src/components/FrameworkDashboard.tsx:494':
    'DISPLAY-ONLY — iteration detail Time: line rendered into the dashboard tree; same judgment as the AdminAnalytics :56 site (display, not export).',
...
describe('dead-idiom batch census (REQ-410)', () => {
  ...
  it('liveness: synthetic fixtures prove every kind detects its incident shape', () => {
    ...
    // the modern spelling alone.
    // (ad1) document.execCommand flagged; the async clipboard API not.
    expect(
      discoverIdiomSites('f.ts', "document.execCommand('copy');"),
    ).toHaveLength(1);
    expect(
      discoverIdiomSites('f.ts', 'await navigator.clipboard.writeText(text);'),
    ).toEqual([]);
    // (ad2) window.clipboardData flagged; the event's own DataTransfer not.
    expect(
    ...
    // (ad3) the deprecated Buffer ctor flagged in BOTH spellings (one
    ...
    // (ad4) url.parse flagged; the WHATWG URL ctor not.
    ...
    // (ad5) the querystring module flagged; URLSearchParams not.
    expect(
    ...
    // (ad6) setImmediate flagged; the portable queueMicrotask not.
    expect(discoverIdiomSites('f.ts', 'setImmediate(flush);')).toHaveLength(1);
    expect(discoverIdiomSites('f.ts', 'queueMicrotask(flush);')).toEqual([]);
    // (ad7) process.nextTick flagged in the isomorphic src tree.
    ...
    // (ad8) Promise.defer()/.defer() flagged; withResolvers not.
    expect(
    ...
    // (ad12) ActiveXObject flagged; a real adapter class not.
    expect(
    ...
    // (ad13) devtools-only console sinks flagged; the logger facade not.
    expect(
    ...
    // (ad14) getComputedStyle's legacy null second arg flagged; the
    ...
    // (ad15) prefixed matches spellings flagged; element.matches not.
    expect(
    ...
    // (ad19) importScripts flagged; the ESM import not.
    expect(
    ...
    // (ad20) Error.captureStackTrace flagged; a plain Error not.
    expect(
    ...
    // (ad21) the self-referential ternary flagged (identifier and
    ...
    // (ad22) the IE-prefixed navigator extension flagged; live navigator
    // members not.
    expect(
    ...
    // (ad23) process.binding flagged; the public import not.
    expect(
    ...
    // (ad24) window.external flagged; a live window member not.
    expect(
    ...
    // (ad25) getBoxObjectFor flagged; getBoundingClientRect not.
    expect(
    ...
    // (ad26) mutation EVENT names flagged; MutationObserver not.
    expect(
      discoverIdiomSites(
        'f.ts',
    ...
    expect(discoverIdiomSites('f.ts', 'new MutationObserver(cb);')).toEqual([]);
    // (ad27) the window.status write flagged; document.title not.
    expect(
    ...
    // (ad28) fireEvent flagged; dispatchEvent not.
    expect(
    ...
    // (ad29) children.map flagged (single-child TypeError shape);
    ...
    // (ad30) Event.path flagged in the evt/e spellings; composedPath and
    ...
    // (ad31) the iv-less cipher ctors flagged; createCipheriv not.
    expect(
      discoverIdiomSites(
        'f.ts',
    ...
    // (ad32) bare toLocale{Date,Time}String flagged; the explicit-locale
    // and ISO forms not.
    expect(
      discoverIdiomSites(
        'f.ts',
    ...
    // (ad33) remove-by-value via indexOf+splice flagged; the filter form
    ...
    // (ad34) the doomed unstable React exports flagged; flushSync not.
    expect(
      discoverIdiomSites(
        'f.ts',
