...
describe('jest-pattern conformance (real @jest/pattern module) — eval follow-up #7', () => {
  ...
  it('claims: ci-run preconditions stay machine-readable — scheme, run id, and PR binding are one grammar (2-phase closure evidence)', () => {
    ...
    expect(ciRunClaimRunIds).toEqual([
      ...
      '2026-09-07T15:00:00Z', // PR #144 — dual-bump 9 回目 (test stage 検証)
      '2026-09-07T16:00:00Z', // PR #145 — dual-bump 10 回目 (test stage 検証)
      '2026-09-07T17:00:00Z', // PR #146 — dual-bump 11 回目 (test stage 検証)
    ]);
  });
  ...
  it('claims: ci-run census binds each run_id to its exact actions run and PR — a rewritten URL REDs, not just a malformed one', () => {
    ...
    expect(bindings).toEqual([
      ...
      ['2026-09-07T15:00:00Z', '34087008200', '144'], // PR #144 — dual bump 9 回目 (test stage 検証)
      ['2026-09-07T16:00:00Z', '34088039373', '145'], // PR #145 — dual bump 10 回目 (test stage 検証)
      ['2026-09-07T17:00:00Z', '34089035460', '146'], // PR #146 — dual bump 11 回目 (test stage 検証)
    ]);
  });
});
