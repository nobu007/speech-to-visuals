 ...
 *   - video-generator's `convertSceneToRemotionFormat` carries a legacy
 ...
 *
 * 1. MIN_SCENE_DURATION_MS — the shared floor. Both the timing optimizer and
 *    the render-spec generator must agree on this: if the render plan floor
 *    were lower than the optimizer floor (or vice versa) the reported video
 *    length would silently diverge from the clamped scene data again.
 *
 * 2. MAX_EDITORIAL_SCENE_DURATION_MS — main-pipeline's pacing cap. Scenes
 *    longer than this are shortened for video flow, even though the renderer
 *    could display them.
 *
 ...
 *
 * Known remaining divergence: video-generator's `convertSceneToRemotionFormat`
 * still clamps the startTime/endTime-derived duration to a legacy [3000, 10000]
 * range (comment in that file explains why it is kept local). Aligning it with
 * MAX_EDITORIAL_SCENE_DURATION_MS would change rendered video timing and needs
 * its own acceptance criteria; until then this module documents the inventory.
 */
