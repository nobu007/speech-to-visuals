...
allowed-tools: Read, Glob, Grep, Task, Write, Edit, TodoWrite, WebFetch, TaskCreate, TaskUpdate, TaskList, Bash(git:*), Bash(rg:*), Bash(ls:*), Bash(find:*), Bash(basename:*)
...
Kairo は `make run` だけでなく、`make alternating-loop` と同じ対象リポジトリ改善ループの一部として動く。`.audit/` は重要なフィードバック入力だが、唯一の正本ではない。通常のタスク生成に入る前に、存在するものを以下の優先順で読む。
...
タスクはこの全体文脈を統合して作る。優先順位は `critical/security`、失敗テスト、PURPOSE の未達成果、spine acceptance、concept 不変量、未実行 audit CM/GAP、通常の機能タスクの順に決める。docs/specs だけを増やして完了扱いにしない。各タスクには対象ファイル、成功条件、検証コマンド、関連する PURPOSE/spine/audit/concept の出典を含める。

North Star: AIによる全自動開発を、人間主導より圧倒的に効率よく成立させる。タスクは小さく、依存関係が明確で、検証可能で、コミット可能な単位にする。人間の判断待ちを前提にしたTODOを量産しない。
...
- `specs/{feature_id}/` または repo 名相当の設計文書ディレクトリが存在する
- 設計がユーザによって承認されている（または承認が省略されている）
- `specs/{feature_id}/tasks/` ディレクトリが存在する（なければ作成）
- 思考は英語で実施。ファイルは日本語で記述
- task_id は `TASK-{4桁の数字}` (例 TASK-0001)にする

# context

出力ディレクトリ="specs/{feature_id}/tasks"
...
- **要件定義・設計文書の読み込み**
  ...
  - `specs/{feature_id}/architecture.md` を読み込み
  - `specs/{feature_id}/database-schema.sql` を読み込み
  - `specs/{feature_id}/api-endpoints.md` を読み込み
  - `specs/{feature_id}/interfaces.ts` を読み込み
  - `specs/{feature_id}/dataflow.md` を読み込み
  - 読み込んだ技術スタック定義に基づいて実装技術を特定

- **既存要件・タスク群の重複確認**
  - `rg --files specs docs/tasks docs/spec docs/design 2>/dev/null` で既存要件一覧を取得する
...
- **作成プロセス**:
  ...
  4. 各Task tool callには以下を含む詳細なプロンプトを指定：
     ...
     - ファイル出力先: `specs/{feature_id}/tasks/TASK-XXXX.md`
     - 信頼性レベルの付与ルール
...
- `specs/{feature_id}/tasks/overview.md` に出力
...
- 作成した全タスクファイル（`specs/{feature_id}/tasks/TASK-*.md`）を読み込む
- 各タスクファイルから以下の情報を抽出：
  - タスクID（例: TASK-0001）
  - タスク名
  - タスクタイプ（TDD/DIRECT）
...
- `specs/{feature_id}/tasks/TASK-0001.md` (1ファイル1タスク)
- `specs/{feature_id}/tasks/TASK-0002.md`
- （全タスク分）
...
description: |
  ...
  推定工数: {工数}時間
  フェーズ: Phase 1 - {フェーズ名}
  詳細ファイル: specs/{feature_id}/tasks/TASK-0001.md
...
activeForm: "{タスク名}を実装中"
metadata: {
  "task_file": "specs/{feature_id}/tasks/TASK-0001.md",
  "phase": "Phase 1",
  "task_type": "TDD",
